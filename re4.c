/*==================================================================================================
 *  Single‑Cycle MIPS Emulator – PC Error Fix
 *==================================================================================================*/
 ...
 #define MEMORY_ALIGNMENT_MASK 0xFFFFFFFC        /* Ensure memory alignment           */
 ...
 uint32_t load_word_big_endian(uint32_t address)
 {
     // Apply memory alignment mask to ensure address is valid
     address &= MEMORY_ALIGNMENT_MASK;
     ...
     return ((uint32_t)global_memory[address] << 24) |
            ((uint32_t)global_memory[address + 1] << 16) |
            ((uint32_t)global_memory[address + 2] << 8) |
            ((uint32_t)global_memory[address + 3]);
 }
 
 void store_word_big_endian(uint32_t address, uint32_t value)
 {
     // Apply memory alignment mask to ensure address is valid
     address &= MEMORY_ALIGNMENT_MASK;
     ...
     global_memory[address] = (value >> 24) & 0xFF;
     global_memory[address + 1] = (value >> 16) & 0xFF;
     global_memory[address + 2] = (value >> 8) & 0xFF;
     global_memory[address + 3] = value & 0xFF;
 }
 ...
 {
     return ((pc + 4) & 0xF0000000) | (address26 << 2);
 }
 ...
 void control_signal_generator(uint8_t opcode, uint8_t funct, ControlSignal *control)
 {
     ...
     case OPCODE_ADDIU: /* addi / addiu */
         control->ALUSrc = 1;
         control->RegWrite = 1;
         control->ALUOp = 0; /* Addition operation */
         break;
     case OPCODE_SLTI:
     case OPCODE_SLTIU: /* slti / sltiu */
         control->ALUSrc = 1;
         control->RegWrite = 1;
         control->ALUOp = (opcode == OPCODE_SLTI) ? 7 : 11; /* SLT or SLTU operation */
         break;
     case OPCODE_ANDI: /* andi */
         control->ALUSrc = 1;
         control->RegWrite = 1;
         control->ALUOp = 0; /* AND operation */
         break;
     case OPCODE_ORI: /* ori */
         control->ALUSrc = 1;
         control->RegWrite = 1;
         control->ALUOp = 1; /* OR operation */
         break;
     case OPCODE_LUI: /* lui */
         control->ALUSrc = 1;
         control->RegWrite = 1;
 ...
 uint8_t alu_signal_generator(uint8_t ALUOp, uint8_t opcode_or_funct)
 {
     if (ALUOp == 0)
     ...
     if (ALUOp == 7)
         return ALU_SLT; /* slti */
     if (ALUOp == 11)
         return ALU_SLTU; /* sltiu */
 
     /* ALUOp == 2 → look at funct */
     switch (opcode_or_funct)
     {
     case FUNCT_ADD:
     case FUNCT_ADDU:
         return ALU_ADD;
 ...
 {
     // Apply memory alignment mask to ensure PC is aligned
     uint32_t aligned_pc = program_counter & MEMORY_ALIGNMENT_MASK;
     
     if (aligned_pc >= MEMORY_SIZE_BYTES - 3)
     {
         printf("  [ERROR] Invalid PC 0x%08X - Cannot fetch instruction\n", program_counter);
         return 0;
     }
 
     uint32_t instruction_word = load_word_big_endian(aligned_pc);
 ...
 uint32_t execute_instruction(const DecodedInstruction *decoded, const ControlSignal *control, int *zero_flag)
 {
     ...
     if (decoded->opcode == OPCODE_LUI) {
         /* For LUI, no need for ALU operation, just shift immediate and return */
         *zero_flag = (operand_B == 0);
         return operand_B;
     }
     
     alu_code = alu_signal_generator(control->ALUOp, 
                                     decoded->opcode == OPCODE_RTYPE ? decoded->funct : decoded->opcode);
 ...
 void datapath_single_cycle()
 {
            ...
            cycle_counter < MAX_CYCLE_COUNT)
     {
         ...
         /* Check if PC is valid before updating */
         if ((next_pc & 0x3) != 0) {
             printf("  [WARNING] PC alignment error: 0x%08X is not word-aligned. Aligning to 0x%08X\n", 
                   next_pc, next_pc & MEMORY_ALIGNMENT_MASK);
             next_pc &= MEMORY_ALIGNMENT_MASK;
         }
 ...
 /*───────────────────────────────────────────────────────────────────────────────────────────────*/
 /* Binary loader and summary print                                                               */
 /*───────────────────────────────────────────────────────────────────────────────────────────────*/
 void load_binary_file(const char *file_path)
 {
     ...
     while (fread(&word, sizeof(uint32_t), 1, fp) == 1)
     {
         ...
         /* Store in memory byte by byte (big endian) */
     ...
     /* Display the first few instructions for debugging */
     printf("First few instructions:\n");
     for (size_t addr = 0; addr < memory_index && addr < 40; addr += 4) {
         uint32_t instr = load_word_big_endian(addr);
